/* ---------------------------------------------------------------------------
   Basic Interpreter
   Robin Edwards 2014
   Modified by Dan Geiger for the SMART Response XE
   ---------------------------------------------------------------------------
   ...
   Dan's notes:
    - Added nonary operator BATT to read battery voltage in mV (print batt)
    - Added BYE to put the unit to sleep. Current=5mA running, 30uA sleeping.
    - Wake the unit with the button on top
    - Added MSAVE and MLOAD. Saves current program to one of 10 memory
      locations (0-9). MLOAD 7
    - Added math function ATN to round out the trig functions.
    - INKEY now does not return a string. It returns the ASCII code of
       the pressed key. This gives access to the function keys. They
       return -16(F0) to -7(F10). If you retrieve INKEY+16, you will
       get the 0-9 value of which Function key was prressed.
...
const char string_25[] PROGMEM = "Error Saving";
const char string_26[] PROGMEM = "Error Loading";
  ...
  if (executeMode) {
    ...
    switch (op) {
      ...
      case TOKEN_LEN:
        ...
        break;
      case TOKEN_SIN:
        if (!stackPushNum(sin(stackPopNum())))
          return ERROR_OUT_OF_MEMORY;
        break;
      case TOKEN_COS:
        if (!stackPushNum(cos(stackPopNum())))
          return ERROR_OUT_OF_MEMORY;
        break;
      case TOKEN_ATN:
        if (!stackPushNum(atan(stackPopNum())))
          return ERROR_OUT_OF_MEMORY;
        break;
      case TOKEN_ABS:
        if (!stackPushNum(abs(stackPopNum())))
          return ERROR_OUT_OF_MEMORY;
        break;
      case TOKEN_LN:
        if (!stackPushNum(log(stackPopNum())))
          return ERROR_OUT_OF_MEMORY;
        break;
      case TOKEN_EXP:
        if (!stackPushNum(exp(stackPopNum())))
          return ERROR_OUT_OF_MEMORY;
        break;
      case TOKEN_SQR:
        if (!stackPushNum(sqrt(stackPopNum())))
          return ERROR_OUT_OF_MEMORY;
        break;
      case TOKEN_SGN:
        tmp = int(stackPopNum());
        if (tmp != 0) tmp = tmp / abs(tmp);
        else tmp = 0;
        if (!stackPushNum(tmp))
          return ERROR_OUT_OF_MEMORY;
        break;
...
int parseBatt() {
  getNextToken();
  uint16_t ba = getBatStat();
  if (executeMode && !stackPushNum((float)ba)) return ERROR_OUT_OF_MEMORY;
  return TYPE_NUMBER;
}
...
int parseMsave() {
  ...
  if (val) return val;  // error
  if (executeMode) {
    short int mNum = (short int)stackPopNum();
    if (mNum < 0 || mNum > 9) return ERROR_BAD_PARAMETER;
    host_saveMem(mNum);
  }
  return 0;
}
